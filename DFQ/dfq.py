...
def _quantize_error(param, num_bits=8, reduction='sum'):
    ...
    with torch.no_grad():
        param_quant = UniformQuantize().apply(param, num_bits, float(param.min()), float(param.max()))#将默认配置改为symmetric=True
        eps = param_quant - param#权重量化误差 epsilon
        if reduction == 'sum':
    ...
    with torch.no_grad():
        ...
        if visualize_state:
            ...
            for rr in relations:
                ...
                visualize_per_layer(graph[layer_first].weight.detach(), '1_before_equalization', index=relation_index)
                visualize_per_layer(graph[layer_second].weight.detach(), '2_before_equalization', index=relation_index)
                relation_index = relation_index+1

        while diff > converge_thres and count < converge_count:#equalize 操作
            num = num + 1
            state_prev = copy.deepcopy(graph)
            relation_index = 0

            for rr in relations:
                # if num>4 and relation_index==2:
                #     continue
                layer_first, layer_second, bn_idx = rr.get_idxs()
                # if visualize_state:#可以画图比较权重分布情况
                #     visualize_per_layer(graph[layer_first].weight.detach(), 'Before equalization',index=relation_index)
                ...
                rr.set_scale_vec(S)
                # if visualize_state:
                #     visualize_per_layer(graph[layer_first].weight.detach(), 'After equalization',index=relation_index)
                relation_index = relation_index + 1
            diff_tmp = 0
            for layer_idx in graph:
                if type(graph[layer_idx]) in targ_type:
                    diff_tmp += float(torch.mean(torch.abs(graph[layer_idx].weight - state_prev[layer_idx].weight)))#diff用来保存各层权重变化的平均情况 变化不大时标志该过程收敛
        ...
        if visualize_state:
            ...
            for rr in relations:
                layer_first, layer_second, bn_idx = rr.get_idxs()
                visualize_per_layer(graph[layer_first].weight.detach(), '1_after_equalization', index=relation_index)
                visualize_per_layer(graph[layer_second].weight.detach(), '2_after_equalization', index=relation_index)
                relation_index = relation_index+1

        print("总共进行了"+str(num)+"次范围均衡操作")
...
def bias_absorption(graph, relations, bottoms, N=3):
    ...
    for rr in relations:
        ...
        num_group = graph[layer_first].weight.size(0) // graph[layer_second].weight.size(1)#第二层的conv分成几组去计算 d-conv分为"输出通道数"组 p-conv分为1组
        step_size_o = size[0] // num_group
        step_size_i = graph[layer_first].weight.size(0) // num_group
        c = (bn_bias - 2 * N * bn_weight)
        # print('均值范围：')
        # print(bn_bias.min(),bn_bias.max())
        # print('方差范围：')
        # print(bn_weight.min(), bn_weight.max())
        d = (bn_bias + 2 * N * bn_weight)
        print("激活值范围：")
        print(c.min(),d.max())
        c.clamp_(0)
        if torch.max(c)>0:
            absorption_works = 1
            print("Absorption works!")
        # S = rr.get_scale_vec()
        # c[S<=1] = 0
        ...
        graph[layer_first].bias.data.add_(-c)#第一层的bias和之后的数据分布都减c
        graph[bn_idx].fake_bias.data.add_(-c)
        graph[layer_second].bias.data.add_(wc)#第二层的偏移加c
    if absorption_works==0:
        print("Absorption doesn't work, all c < 0.")
        return absorption_works
    ...
    with torch.no_grad():
        for idx_layer in graph:
            ...
            if type(graph[idx_layer]) in targ_type: # 1 to many or 1 to 1
                ...
                eps = _quantize_error(weight, bits_weight, reduction=None)
                eps = torch.sum(eps.view(weight.size(0), weight.size(1), -1), -1)#将权重量化误差写成[output_channel, input_channel]的形式
                ...
                # group operation
                num_group = expect.size(0) // eps.size(1)#分成几组进行conv计算 p-conv是1组 d-conv是输出通道组
                step_size_o = eps.size(0) // num_group
                step_size_i = expect.size(0) // num_group

                bias = torch.zeros(eps.size(0))
                for g in range(num_group):#y=wx+b wx-> [output_channel,input_channel]*[input_channel]+[output_channel]
                ...
                graph[idx_layer].bias.add_(-bias)#把由于权重量化所引起的误差减去
                bias_prev = -bias#由于偏差纠正导致其后BN层的统计特征发生变化

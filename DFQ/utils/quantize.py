...
class UniformQuantize(InplaceFunction):
    ...
    """
    @staticmethod
    def forward(ctx, input, num_bits=8, min_value=None, max_value=None, inplace=False, symmetric=True, num_chunks=None):#把默认量化改为对称量化
        ...
        else:  # 对于永远为正数的激活值量化 采用非对称的能增加一倍的表示范围
            qmin = 0.
            qmax = 2. ** num_bits - 1.

            scale = (max_value - min_value) / (qmax - qmin)
            
        scale = max(scale, 1e-8)  # scale最小不能小于1e-8
        # test = np.array([scale])
        # print("test", test.dtype)

        output.add_(-min_value).div_(scale)  # 这里其实是非对称量化
...
def quantize(x, num_bits=8, min_value=None, max_value=None, inplace=False, symmetric=True, num_chunks=None):
...
class QuantMeasure(nn.Module):
    ...
    def __init__(self, update_stat=False, num_bits=8, momentum=0.1):
        ...
        if torch.cuda.is_available():
            ...
            device = 'cpu'
        self.register_buffer('running_min', torch.zeros(1).to(device))#增加 running_min 和 running_max 属性保存激活值范围
        self.register_buffer('running_max', torch.zeros(1).to(device))
        self.momentum = momentum
        self.num_bits = num_bits
        self.update_stat = update_stat
        self.running_max_list = []#用来对activation做perchannel的量化
        self.running_min_list = []
    ...
    def forward(self, input):
        ...
        return quantize(input, self.num_bits, min_value=float(min_value), max_value=float(max_value), num_chunks=16, symmetric=False)#把激活值的量化变为无符号数 因为其恒大于0
...
class QuantNConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        super(QuantNConv2d, self).__init__(in_channels, out_channels, kernel_size,
                                      stride, padding, dilation, groups, bias)
        self.quant = QuantMeasure(num_bits=num_bits_act, momentum=momentum)#量化的conv相比于conv多了一个卷积计算前的QuantMeasure()操作对激活值做量化
...
def set_layer_bits(graph, bits_weight=8, bits_activation=8, bits_bias=16, targ_type=None):
    ...
    for idx in graph:
        if type(graph[idx]) in targ_type:
            if hasattr(graph[idx], 'quant'):#set num_bits for quant_layer 其实就对 activation 有用

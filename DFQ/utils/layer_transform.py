...
def replace_op():
    ...
    # print(globals()['___add__'])
    for op_name in tensor_magic_op_supported:
        raw_op = getattr(torch.Tensor ,op_name)
        raw_tensor_magic_op[op_name] = raw_op#记录原始的op操作
        setattr(tensor_target, op_name, globals()['_' + op_name])#把op换成对应的量化op，给 tensor_target 的属性 op_name 设置值，为当前全局变量字典中 _+op_name 项所对应的 value
                ...
                if type(graph[layer_idx]) == nn.BatchNorm2d and type(graph[bot_idx]) in targ_type:
                    ...
                    if graph[bot_idx].bias is None: # add a bias term to conv or linear layer
                        if torch.cuda.is_available():
                            ...
                            device = 'cpu'
                        graph[bot_idx].bias = nn.Parameter(data=torch.zeros((graph[bot_idx].weight.size(0)), dtype=torch.float32).to(device), requires_grad=False)
...
def quantize_targ_layer(graph, bit_weight=8, bits_bias=16, targ_type=None):
    ...
    for layer_idx in graph:
        if type(graph[layer_idx]) in targ_type:# QLinear 和 Qconv 层中的参数做量化
            with torch.no_grad():
    ...
    for idx_layer in graph:
        ...
        if len(bot) == 1 and bot[0] == 'Data':
            if is_detection:
                ...
                quant_module[0].running_max.fill_(1)# max & min for activation
                quant_module[0].running_min.fill_(0)
                for i in range(3):
                    graph[idx_layer].quant.running_max_list.append(1)
                    graph[idx_layer].quant.running_min_list.append(0)
            ...
            if len(quant_module) == len(bn_list): # 1 to 1 mapping
                ...
                while idx < len(bn_list):
                    ...
                    if bn_list[idx][1][0] in targ_without_bn:
                        ...
                        value_min_torch = torch.clamp(bias - N * weight,min=0) if 'relu' in relu_attach_list[idx] else bias - N * weight
                        value_max_torch = torch.clamp(bias + N * weight,min=0,max=6) if 'relu6' in relu_attach_list[idx] else torch.clamp(bias + N * weight,min=0)
                    ...
                    # quant_module[idx].running_max = value_max
                    # quant_module[idx].running_min = value_min
                    # for item in value_max_torch:
                    #     quant_module[idx].running_max_list.append(float(item))
                    # for item in value_min_torch:
                    #     quant_module[idx].running_min_list.append(float(item))
                    if idx_layer!='torch.cat_35':
                        for item in value_max_torch:
                            graph[idx_layer].quant.running_max_list.append(float(item))
                        for item in value_min_torch:
                            graph[idx_layer].quant.running_min_list.append(float(item))
                ...
                for key in bn_branch:
                    ...
                    if 'add' in connect_type:
                        ...
                        value_min_torch_1 = torch.clamp(bias - N * weight, min=0) if 'relu' in relu_attach_list[idx] else bias - N * weight
                        value_max_torch_1 = torch.clamp(bias + N * weight, min=0, max=6) if 'relu6' in relu_attach_list[idx] else torch.clamp(bias + N * weight, min=0)
                            ...
                            for idx in range(idx_bound):
                                ...
                                if 'add' in connect_type:
                                    ...
                                    if 'cat' == connect_type:
                                        ...
                                        value_min_torch_2 = torch.clamp(bias - N * weight, min=0) if 'relu' in relu_attach_list[idx] else bias - N * weight
                                        value_max_torch_2 = torch.clamp(bias + N * weight, min=0, max=6) if 'relu6' in relu_attach_list[idx] else torch.clamp(bias + N * weight, min=0)
                                    else:
                ...
                if len(quant_module) == 1 and len(quant_module) < len(bn_list): # 1 to many
                    ...
                    for i in range(4):
                       for item in value_max_torch_1:
                            graph[idx_layer].quant.running_max_list.append(float(item))
                    for item in value_max_torch_2:
                        graph[idx_layer].quant.running_max_list.append(float(item))
                    for i in range(4):
                        for item in value_min_torch_1:
                            graph[idx_layer].quant.running_min_list.append(float(item))
                    for item in value_min_torch_2:
                        graph[idx_layer].quant.running_min_list.append(float(item))
